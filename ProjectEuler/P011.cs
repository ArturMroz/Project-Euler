...
namespace ProjectEuler
{
    ...
    public static class P011
    {
        ...
        {
            // products: horizontal, vertical, diagonal up-down, diagonal down-up
            int hProduct, vProduct, dUpProduct, dDownProduct;
            int maxProduct = 1;
            ...
            for (int i = 0; i < SIZE; i++)
            {
                ...
                {
                    hProduct = vProduct = dUpProduct = dDownProduct = 1;

                    for (int k = 0; k < 4; k++)
                    {
                        hProduct *= table[i, j + k];
                        vProduct *= table[j + k, i];

                        if (i > SIZE - 4) continue;
                        dUpProduct *= table[i + k, j + k];
